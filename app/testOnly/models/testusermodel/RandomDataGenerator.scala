...
object RandomDataGenerator {
   ...
   * @return hopefully a valid Vrn
   */
  def nextVrn()(implicit r: Random): Vrn = {
      // Generates a valid VRN with the option of specifying specific digits for the first seven digits of the VRN
      def generateVRN(v1: Option[Int], v2: Option[Int], v3: Option[Int], v4: Option[Int], v5: Option[Int], v6: Option[Int], v7: Option[Int]): Vrn = {
  ...
  def nextForename()(implicit r: Random): String = choose(
    ...
    "Jackson", "Aiden", "Lucas", "Liam", "Noah", "Ethan", "Mason", "Caden", "Oliver", "Elijah", "Grayson", "Jacob", "Michael", "Benjamin", "Carter", "James", "Jayden", "Logan", "Alexander", "Caleb", "Ryan", "Luke", "Daniel", "Jack", "William", "Owen", "Gabriel", "Matthew", "Connor", "Jayce", "Isaac", "Sebastian", "Henry", "Muhammad", "Cameron", "Wyatt", "Dylan", "Nathan", "Nicholas", "Julian", "Eli", "Levi", "Isaiah", "Landon", "David", "Christian", "Andrew", "Brayden", "John", "Andy", "Kenny", "Steve"
  )

  def nextSurname()(implicit r: Random): String = choose(
    "Williams", "Johnson", "Taylor", "Thomas", "Roberts", "Khan", "Lewis", "Jackson", "Clarke", "James", "Phillips", "Wilson", "Ali", "Mason", "Mitchell", "Rose", "Davis", "Davies", "Rodriguez", "Cox", "Alexander", "Morgan", "Moore", "Mills", "King", "Adams", "Garcia", "White", "Stone", "Edwards", "Watson", "Mallen", "Walker", "Austin", "Pearce", "Reid", "Simon", "Chung", "Vo"
  )
  ...
  def nextSortCode()(implicit r: Random): String = nextNumber(6)
  def nextAccountNumber()(implicit r: Random): String = nextNumber(8)

  def nextDomain()(implicit r: Random): String = choose(
    "google.test",
    "yahoo.test",
    "easy.email.test"
  )
  ...
  def nextBoolean()(implicit r: Random): Boolean = choose(true, false)

  def nextAlphanumeric(n: Int)(implicit r: Random): String = r.alphanumeric.take(n).mkString

  def nextHex(n: Int)(implicit r: Random): String = r.alphanumeric.filter(x => x.isDigit || "abcdefABCDEF".contains(x)).take(n).mkString

  def nextAlpha(n: Int)(implicit r: Random): String = r.alphanumeric.filter(_.isLetter).take(n).mkString
  /**
   * Next n-digit number string. Values can start from '0'
   */
  def nextNumber(n: Int)(implicit r: Random): String = r.alphanumeric.filter(_.isDigit).take(n).mkString

  def nextIntBetween(i: Int, j: Int): Int = i + Random.nextInt(j - i + 1)

  def nextPositiveLong(n: Int)(implicit random: Random): Long = {
