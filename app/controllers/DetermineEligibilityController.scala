    ...
    val maybeEligibilityCheckResult: OptionT[Future, EligibilityCheckResult] = for {
      ...
      if (eligibilityCheckResult.isEligible) {
        journey.taxRegime match {
          case Sa if eligibilityCheckResult.individualDetails.flatMap(_.customerType).contains(CustomerTypes.MTDITSA) =>
            if (r.enrolments.getEnrolment("HMRC-MTD-IT").isDefined)
