...
class BarsVerifyBusinessRequestSpec extends UnitSpec {

  "BarsVerifyBusinessRequest" - {

    "must have a format instance" in {
      val request = BarsVerifyBusinessRequest(
        BarsBankAccount(
          "123456",
          "12345678"
        ),
      ...
      expectedJson.validate[BarsVerifyBusinessRequest] shouldBe JsSuccess(request)
    }
