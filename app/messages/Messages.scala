...
object Messages {
  ...
  object YourBill {
    ...
    def Due(date: LocalDate): Message = {
      ...
      val welshDateString   =
        s"${day.toString}${DateMessages.getSuffix(day)(using Languages.Welsh)} " +
          s"${DateMessages.monthName(date.getMonthValue).show(using Languages.Welsh)} " +
    ...
    def `ChargeTypeRow`(mTrans: MainTrans, taxYearStartYear: Int, taxYearEndYear: Int): Message = {
      ...
      val chargeTypeDescriptionEnglish: String = ChargeTypeMessages.chargeFromMTrans
        .get(mTrans)
        .map(_.english)
        .getOrElse(
      ...
      val chargeTypeDescriptionWelsh: String = ChargeTypeMessages.chargeFromMTrans
        .get(mTrans)
        .map(_.show(using Languages.Welsh))
        .getOrElse(
          throw MainTrans.UnknownMainTransException(mTrans)
        )
  ...
  object ExtraSupport {
    ...
    def `Find out the different ways to deal with HMRC...`(link: String): Message = Message(
      english =
        s"""Find out the different ways to <a href="$link" class="govuk-link">deal with HMRC if you need some help</a>.""",
      welsh =
        s"""Dysgwch am y ffyrdd gwahanol o <a href="$link" class="govuk-link">ddelio â CThEF os oes angen help arnoch chi</a>."""
    )
    ...
    def `You can also use Relay UK...`(link: String): Message = Message(
      ...
      welsh =
        s"""Gallwch hefyd ddefnyddio <a href="$link" class="govuk-link">Relay UK</a> os na allwch glywed na siarad dros y ffôn: deialwch <strong>18001</strong> ac yna <strong>0345 300 3900</strong>. Sylwer – dim ond galwadau ffôn Saesneg eu hiaith y mae Relay UK yn gallu ymdrin â nhw."""
    )
  ...
  object NotEligible {
    ...
    val `Our opening times are Monday to Friday, 8am to 6pm. We are closed on weekends and bank holidays.` : Message =
      Message(
    ...
    def `You cannot set up ... debt too large`(taxRegime: TaxRegime, maxAmountOfDebt: AmountInPence): Message =
      taxRegime match {
        case TaxRegime.Epaye =>
          Message(
            english =
              s"You cannot set up an Employers’ PAYE payment plan online because you owe more than ${maxAmountOfDebt.gdsFormatInPounds}.",
            welsh =
              s"Ni allwch drefnu cynllun talu TAW ar-lein oherwydd mae arnoch dros ${maxAmountOfDebt.gdsFormatInPounds}."
          )
        case TaxRegime.Vat   =>
          Message(
            english =
              s"You cannot set up a VAT payment plan online because you owe more than ${maxAmountOfDebt.gdsFormatInPounds}.",
            welsh =
              s"Ni allwch drefnu cynllun talu ar gyfer TWE Cyflogwyr ar-lein oherwydd mae arnoch dros ${maxAmountOfDebt.gdsFormatInPounds}."
          )
        case TaxRegime.Sa    =>
          Message(
            english =
              s"You cannot set up a Self Assessment payment plan online because you owe more than ${maxAmountOfDebt.gdsFormatInPounds}.",
            welsh =
              s"Ni allwch drefnu cynllun talu Hunanasesiad ar-lein oherwydd mae arnoch dros ${maxAmountOfDebt.gdsFormatInPounds}."
          )
        case TaxRegime.Simp  =>
          Message(
            english =
              s"You cannot set up a Simple Assessment payment plan online because you owe more than ${maxAmountOfDebt.gdsFormatInPounds}.",
            welsh =
              s"Ni allwch drefnu cynllun talu Asesiad Syml ar-lein oherwydd mae arnoch dros ${maxAmountOfDebt.gdsFormatInPounds}."
          )
      }
    ...
    def `Make a payment online to cover your ... bill in full.`(taxRegime: TaxRegime, link: String): Message =
      taxRegime match {
        case TaxRegime.Epaye =>
          Message(
            english =
              s"""<a class="govuk-link" href="$link">Make a payment online</a> to cover your PAYE bill in full.""",
            welsh = s"""<a class="govuk-link" href="$link">Gwnewch daliad ar-lein</a> i dalu’ch bil TWE yn llawn."""
          )
        case TaxRegime.Vat   =>
          Message(
            english =
              s"""<a class="govuk-link" href="$link">Make a payment online</a> to cover your VAT bill in full.""",
            welsh = s"""<a class="govuk-link" href="$link">Gwnewch daliad ar-lein</a> i dalu’ch bil TAW yn llawn."""
          )
        case TaxRegime.Sa    =>
          Message(
            english =
              s"""<a class="govuk-link" href="$link">Make a payment online</a> to cover your Self Assessment tax bill in full.""",
            welsh =
              s"""<a class="govuk-link" href="$link">Gwnewch daliad ar-lein</a> i dalu’ch bil Hunanasesiad yn llawn."""
          )
        case TaxRegime.Simp  =>
          Message(
            english =
              s"""<a class="govuk-link" href="$link">Make a payment online</a> to cover your Simple Assessment tax bill in full.""",
            welsh =
              s"""<a class="govuk-link" href="$link">Gwnewch daliad ar-lein</a> i dalu’ch bil treth Asesiad Syml yn llawn."""
          )
      }
    ...
    def `You cannot set up ... debt too old`(taxRegime: TaxRegime, ageOfDebtInYearsOrDays: Int): Message =
      taxRegime match {
        // years
        case TaxRegime.Epaye =>
          Message(
            english =
              s"You cannot set up an Employers’ PAYE payment plan online because your payment deadline was over ${ageOfDebtInYearsOrDays.toString} years ago.",
            welsh =
              s"Ni allwch drefnu cynllun talu ar gyfer TWE Cyflogwyr ar-lein oherwydd roedd y dyddiad cau ar gyfer talu dros ${ageOfDebtInYearsOrDays.toString} mlynedd yn ôl."
          )
        // days (for now)
        case TaxRegime.Vat   =>
          Message(
            english =
              s"You cannot set up a VAT payment plan online because your payment deadline was over ${ageOfDebtInYearsOrDays.toString} days ago.",
            welsh =
              s"Ni allwch drefnu cynllun talu TAW ar-lein oherwydd roedd y dyddiad cau ar gyfer talu dros ${ageOfDebtInYearsOrDays.toString} wythnos yn ôl."
          )
        case TaxRegime.Sa    =>
          Message(
            english =
              s"You cannot set up a Self Assessment payment plan online because your payment deadline was over ${ageOfDebtInYearsOrDays.toString} days ago.",
            welsh =
              s"Ni allwch drefnu cynllun talu Hunanasesiad ar-lein oherwydd roedd y dyddiad cau ar gyfer talu dros ${ageOfDebtInYearsOrDays.toString} diwrnod yn ôl."
          )
        case TaxRegime.Simp  =>
          Message(
            english =
              s"You cannot set up a Simple Assessment payment plan online because your payment deadline was over ${ageOfDebtInYearsOrDays.toString} days ago.",
            welsh =
              s"Ni allwch drefnu cynllun talu Asesiad Syml ar-lein oherwydd roedd y dyddiad cau ar gyfer talu dros ${ageOfDebtInYearsOrDays.toString} diwrnod yn ôl."
          )
      }

    def `You cannot set up ... accounting period that started before`(accountingPeriodStart: String): Message = Message(
      english =
        s"You cannot set up a VAT payment plan online because your debt is for an accounting period that started before $accountingPeriodStart.",
      welsh =
        s"Ni allwch drefnu cynllun talu TAW ar-lein oherwydd bod eich dyled am gyfnod cyfrifyddu a ddechreuodd cyn $accountingPeriodStart."
    )
    ...
    val `You cannot set up an Employers' PAYE payment plan online`: Message = Message(
      english =
        "You cannot set up an Employers’ PAYE payment plan online because you already have a payment plan with HMRC.",
      welsh =
        "Ni allwch drefnu cynllun talu ar-lein ar gyfer TWE y Cyflogwr oherwydd bod gennych gynllun talu ar-lein gyda CThEF yn barod."
    )
    ...
    val `You cannot set up a VAT payment plan online.` : Message = Message(
      ...
      welsh =
        "Ni allwch drefnu cynllun talu ar-lein ar gyfer TAW oherwydd bod gennych gynllun talu gyda CThEF yn barod."
    )

    val `You cannot set up a Self Assessment payment plan online.` : Message = Message(
      english =
        "You cannot set up a Self Assessment payment plan online because you already have a payment plan with HMRC.",
      welsh =
        "Ni allwch drefnu cynllun talu ar-lein ar gyfer Hunanasesiad oherwydd bod gennych gynllun talu gyda CThEF yn barod."
    )

    val `You cannot set up a Simple Assessment payment plan online.` : Message = Message(
      english =
        "You cannot set up a Simple Assessment payment plan online because you already have a payment plan with HMRC.",
      welsh =
        "Ni allwch drefnu cynllun talu ar-lein ar gyfer Asesiad Syml oherwydd bod gennych gynllun talu gyda CThEF yn barod."
    )
    ...
    def `Generic RLS message`(taxRegime: TaxRegime): Message = taxRegime match {
      case TaxRegime.Epaye =>
        Message(
          english =
            "You cannot set up an Employers’ PAYE payment plan online because some of your personal details are not up to date.",
          welsh =
            "Ni allwch drefnu cynllun talu ar gyfer TWE y Cyflogwr ar-lein oherwydd nad yw rhai o’ch manylion personol yn gyfredol."
        )
      case TaxRegime.Vat   =>
        Message(
          english =
            "You cannot set up a VAT payment plan online because some of your personal details are not up to date.",
          welsh = "Ni allwch drefnu cynllun talu TAW ar-lein oherwydd nad yw rhai o’ch manylion personol yn gyfredol."
        )
      case TaxRegime.Sa    =>
        Message(
          english =
            "You cannot set up a Self Assessment payment plan online because some of your personal details are not up to date.",
          welsh =
            "Ni allwch drefnu cynllun talu Hunanasesiad ar-lein oherwydd nad yw rhai o’ch manylion personol yn gyfredol."
        )
      case TaxRegime.Simp  =>
        Message(
          english =
            "You cannot set up a Simple Assessment payment plan online because some of your personal details are not up to date.",
          welsh =
            "Ni allwch drefnu cynllun talu Asesiad Syml ar-lein oherwydd nad yw rhai o’ch manylion personol yn gyfredol."
        )
    }

    def `You must update your details with HMRC`(link: String): Message = Message(
      english =
        s"""You must <a href="$link" class="govuk-link">update your details with HMRC</a>. After you’ve updated your details, wait 3 working days before trying again online.""",
      welsh =
        s"""Mae’n rhaid i chi <a href="$link" class="govuk-link">roi’ch manylion newydd i CThEF</a>. Ar ôl i chi diweddaru’ch manylion, arhoswch 3 diwrnod gwaith cyn rhoi tro arall arni ar-lein."""
    )
    ...
    val `If you have recently filed your return, your account can take up to 3 days to update. Try again after 3 days.`
      ...
      welsh =
        "Os ydych wedi cyflwyno’ch Ffurflen Dreth yn ddiweddar, gall gymryd hyd at 3 diwrnod i ddiweddaru’ch cyfrif. Rhowch gynnig arall arni ar ôl 3 diwrnod."
    )
    ...
    def `You must file your tax return before you can set up an Employers’ PAYE payment plan online`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a class="govuk-link" href="$fileReturnUrl">gyflwyno’ch Ffurflen Dreth</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer TWE y Cyflogwr."""
    )
    ...
    def `You must file your tax return before you can set up a VAT payment plan online`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a class="govuk-link" href="$fileReturnUrl">gyflwyno’ch Ffurflen Dreth</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer TAW."""
    )
    ...
    def `You must file your tax return before you can set up a Self Assessment payment plan online`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a class="govuk-link" href="$fileReturnUrl">gyflwyno’ch Ffurflen Dreth</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer Hunanasesiad ar-lein."""
    )
    ...
    def `You must file your tax return before you can set up a Simple Assessment payment plan online`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a class="govuk-link" href="$fileReturnUrl">gyflwyno’ch Ffurflen Dreth</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer Asesiad Syml ar-lein."""
    )
    ...
    val `You may be able to set up a payment plan once the deadline...` : Message = Message(
      ...
      welsh =
        "Efallai y byddwch yn gallu trefnu cynllun talu unwaith y bydd y dyddiad cau wedi mynd heibio i dalu’ch bil."
    )
  ...
  object EnrolmentMissing {
    ...
    def `You must enrol for PAYE Online before you can set up an Employers’ PAYE payment plan.`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a href="$payeLink" class="govuk-link">ymrestru ar gyfer TWE Ar-lein</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer TWE y Cyflogwr."""
    )
    ...
    def `You must register for VAT online before you can set up a VAT payment plan.`(vatLink: String): Message =
      Message(
        english =
          s"""You must <a href="$vatLink" class="govuk-link">register for VAT online</a> before you can set up a VAT payment plan online.""",
        welsh =
          s"""Mae’n rhaid i chi <a href="$vatLink" class="govuk-link">gofrestru ar gyfer TAW ar-lein</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer TAW."""
      )
    ...
    def `You must request access to Self Assessment before you can set up a Self Assessment payment plan.`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a href="$saLink" class="govuk-link">wneud cais i gael mynediad at eich cyfrif Hunanasesiad</a> cyn i chi allu trefnu cynllun talu ar-lein ar gyfer Hunanasesiad."""
    )

    val `If you already have access, sign in with the Government Gateway user ID that has your enrolment.` : Message =
      Message(
        english = "If you already have access, sign in with the Government Gateway user ID that has your enrolment.",
        welsh =
          "Os oes gennych fynediad yn barod, mae’n rhaid i chi fewngofnodi gan ddefnyddio’r Dynodydd Defnyddiwr (ID) Porth y Llywodraeth sydd â’ch cofrestriad."
      )
    ...
    val `If you’ve already signed up, sign in with the Government Gateway user ID that has your enrolment.` : Message =
      Message(
    ...
    def `You must sign up for Making Tax Digital for Income Tax before you can set up a Self Assessment payment plan online.`(
      ...
      welsh =
        s"""Mae’n rhaid i chi <a href="$mtdLink" class="govuk-link">gofrestru ar gyfer y cynllun Troi Treth yn Ddigidol ar gyfer Treth Incwm</a> cyn i chi allu trefnu cynllun talu ar gyfer Hunanasesiad ar-lein."""
    )
  }
  ...
  object UpfrontPayment {
    ...
    val `An upfront payment is separate to any recent payments you've made...` : Message = Message(
      ...
      welsh =
        "Mae taliad ymlaen llaw ar wahân i unrhyw daliadau diweddar yr ydych wedi eu gwneud. Byddwn yn ei gymryd o’ch cyfrif banc cyn pen 6 diwrnod gwaith."
    )

    val `If you pay some of your bill upfront, you'll have a shorter payment plan.` : Message = Message(
      english = "If you pay some of your bill upfront, you’ll have a shorter payment plan.",
      welsh = "Os byddwch yn talu rhywfaint o’ch bil ymlaen llaw, bydd gennych gynllun talu byrrach."
    )

    val `This payment will be taken from your bank account within 6 working days.` : Message = Message(
      english = "This payment will be taken from your bank account within 6 working days.",
      welsh = "Caiff y taliad hwn ei gymryd o’ch cyfrif banc cyn pen 6 diwrnod gwaith."
    )

    val `An upfront payment is separate to any recent payments you have made.` : Message = Message(
  ...
  object CallHmrc {
    val `If you do not think you can set up a plan online, call HMRC and find out if you can set up a plan over the phone.`
      ...
      welsh =
        "Os nad ydych yn credu y gallwch sefydlu cynllun ar-lein, ffoniwch CThEF a dysgwch a allwch sefydlu cynllun dros y ffôn."
    )
    ...
    def `Our guidance Get help from HMRC...`(link: String): Message = Message(
      english =
        s"""Our guidance <a href="$link" class="govuk-link" rel="noreferrer noopener" target="_blank">Get help from HMRC if you need extra support (opens in new tab)</a> explains how we can support you.""",
      welsh =
        s"""Bydd ein harweiniad ynghylch <a href="$link" class="govuk-link" rel="noreferrer noopener" target="_blank">‘Cael help gan CThEF os oes angen cymorth ychwanegol arnoch’ (yn agor tab newydd)</a> yn esbonio sut y gallwn eich helpu."""
    )
  }
  ...
  object Epaye {
    ...
    val `Use this service to set up a payment plan..` : Message = Message(
      english =
        "Use this service to set up a payment plan for your outstanding employers’ PAYE bill. Payments are taken by Direct Debit and include interest charged at the Bank of England base rate plus 2.5% per year.",
      welsh =
        "Defnyddiwch y gwasanaeth hwn i sefydlu cynllun talu ar gyfer eich bil TWE y cyflogwyr sy’n weddill. Mae taliadau’n cael eu cymryd drwy Ddebyd Uniongyrchol ac maent yn cynnwys llog a godir ar gyfradd sylfaenol Banc Lloegr ynghyd â 2.5% y flwyddyn."
    )
    ...
    val `have no outstanding employers’ PAYE submissions or Construction Industry Scheme returns`: Message = Message(
      ...
      welsh =
        "nid oes gan eich cwmni neu’ch partneriaeth unrhyw gyflwyniadau TWE y cyflogwr sy’n weddill neu Ffurflenni Treth Cynllun y Diwydiant Adeiladu"
    )
  ...
  object Vat {
    ...
    val `Use this service to set up a payment plan..` : Message = Message(
      english =
        "Use this service to set up a payment plan for your outstanding VAT bill. Payments are taken by Direct Debit and include interest charged at the Bank of England base rate plus 2.5% per year.",
      welsh =
        "Defnyddiwch y gwasanaeth hwn i drefnu cynllun talu ar gyfer eich bil TAW sy’n weddill. Mae taliadau’n cael eu cymryd drwy Ddebyd Uniongyrchol ac maent yn cynnwys llog a godir ar gyfradd sylfaenol Banc Lloegr ynghyd â 2.5% y flwyddyn."
    )
    ...
    val `If you have a Customer Compliance Manager...` : Message = Message(
      ...
      welsh =
        "Os oes gennych chi reolwr cydymffurfiad cwsmeriaid, trafodwch eich anghenion ag ef cyn defnyddio’r gwasanaeth hwn."
    )
  ...
  object Simp {
    ...
    val `You must keep up to date with your payments...` : Message = Message(
      english =
        "You must keep up to date with your payments. HMRC may ask you to pay the total outstanding amount if you do not. " +
          "HMRC intend this as a one off payment plan to give you extra support.",
      welsh =
        "Mae’n rhaid i chi sicrhau eich bod chi’n gwneud eich taliadau mewn pryd. Mae’n bosibl y bydd CThEF yn gofyn i chi dalu’r cyfanswm sy’n ddyledus os na fyddwch chi’n gwneud eich taliadau mewn pryd. Bwriad CThEF yw y bydd hwn yn gynllun talu untro er mwyn rhoi cymorth ychwanegol i chi."
    )
  ...
  object MonthlyPaymentAmount {
    ...
    val `You may still be able to set up a payment plan...` : Message = Message(
      english =
        "You may still be able to set up a payment plan over the phone. Call us on <strong>0300 123 1813</strong> to speak to an adviser.",
      welsh =
        "Mae’n bosibl y byddwch chi’n dal i allu trefnu cynllun talu dros y ffôn. Ffoniwch ni ar <strong>0300 200 1900</strong> i siarad ag ymgynghorydd."
    )
  ...
  object Instalments {
    ...
    val `If interest rates change...` : Message = Message(
      english =
        "If the interest rate changes during your payment plan, you may need to settle any difference at the end. " +
          "We will contact you if this is the case.",
      welsh =
        "Os bydd y gyfradd llog yn newid yn ystod eich cynllun talu, efallai bydd yn rhaid i chi setlo unrhyw wahaniaeth ar y diwedd. " +
  ...
  object AboutYourBankAccount {
    ...
    val `authorised to set up a Direct Debit...` : Message = Message(
      ...
      welsh =
        "mae’n rhaid eich bod wedi’ch awdurdodi i sefydlu Debyd Uniongyrchol heb lofnod gan unrhyw ddeiliaid cyfrif eraill"
    )
      ...
      Map(
        ...
        "name.error.disallowedCharacters" -> { disallowedCharacters =>
          Message(
            ...
            welsh =
              s"Mae’n rhaid i’r enw sydd ar y cyfrif peidio â chynnwys ${commaSeparateList(disallowedCharacters, "neu")}"
          )
        },
  ...
  object TermsAndConditions {
    ...
    def `Use Relay UK...`(link: String): Message = Message(
      english =
        s"""Use Relay UK if you cannot hear or speak on the telephone, dial <strong>18001</strong> then <strong>0345 300 3900</strong>. Find out more on the <a href="$link" class="govuk-link" rel="noreferrer noopener" target="_blank">Relay UK website (opens in new tab)</a>.""",
      welsh =
        s"""Defnyddiwch wasanaeth Text Relay UK os na allwch glywed na siarad dros y ffôn. Deialwch <strong>18001</strong> ac yna <strong>0345 300 3900</strong>. Dysgwch ragor am hyn ar <a href="$link" class="govuk-link" rel="noreferrer noopener" target="_blank">wefan Text Relay UK (yn agor tab newydd)</a>."""
    )
  ...
  object EmailEntry {
    ...
    val `We will use this email address to...` : Message = Message(
      english =
        "We will use this email address to send you information about your payment plan. It may take <strong>up to 24 hours</strong> to receive notifications after you set up your plan.",
      welsh =
        "Byddwn yn defnyddio’r cyfeiriad e-bost hwn er mwyn anfon gwybodaeth atoch am eich cynllun talu. Gallai gymryd <strong>hyd at 24 awr</strong> i gael hysbysiadau ar ôl i chi drefnu’ch cynllun."
    )
  ...
  object TooManyEmails {
    ...
    def `You have been locked out because you have tried to verify too many email addresses`(
      ...
      welsh =
        s"""Rydych chi wedi cael eich cloi allan oherwydd eich bod wedi ceisio dilysu gormod o gyfeiriadau e-bost. Rhowch gynnig arall arni ar <strong>$dateAndTime</strong>."""
    )
  ...
  object NotSoleSignatory {
    ...
    val `You cannot set up a Self Assessment payment plan online if you are not the only account holder.` : Message =
      Message(
    ...
    val `You cannot set up a Simple Assessment payment plan online if you are not the only account holder.` : Message =
      Message(
    ...
    val `You cannot set up an Employers’ PAYE payment plan online if you are not the only account holder.` : Message =
      Message(
    ...
    val `Call us on 0300 123 1813 if you need to set up a Direct Debit from a joint account. All account holders must be present when calling.`
      ...
      welsh =
        "Ffoniwch ni ar <strong>0300 200 1900</strong> os oes angen trefnu Debyd Uniongyrchol o gyfrif ar y cyd. Mae’n rhaid i holl ddeiliaid y cyfrif fod yn bresennol pan fyddwch yn ffonio."
    )
  ...
  object BankDetailsLockout {
    ...
    val `For further support you can contact the Payment Support Service...` : Message = Message(
      ...
      welsh =
        "I gael cymorth pellach, gallwch gysylltu â Gwasanaeth Cwsmeriaid Cymraeg CThEF ar <strong>0300 200 1900</strong> i siarad ag ymgynghorydd."
    )
  ...
  object Confirmation {
    ...
    val `If you've made an upfront payment, we'll take it from your bank account within 6 working days.` : Message =
      Message(
        ...
        welsh =
          "Os ydych wedi gwneud taliad ymlaen llaw, byddwn yn ei gymryd o’ch cyfrif banc cyn pen 6 diwrnod gwaith."
      )
    ...
    val `We will not send you a copy`: Message = Message(
      english =
        "<strong>We will not send a copy of your payment plan in the post. This is your only chance to access this information.</strong>",
      welsh =
        "<strong>Ni fyddwn yn anfon copi o’ch cynllun talu drwy’r post. Dyma’ch unig gyfle i gael mynediad at yr wybodaeth hon.</strong>"
    )
    ...
    def paymentInfo(hasUpfrontPayment: Boolean, paymentDate: String): Message = Message(
      english =
        s"${if (hasUpfrontPayment) "Your upfront payment will be taken within 6 working days. " else ""}Your next payment will be taken on $paymentDate or the next working day.",
      welsh =
        s"${if (hasUpfrontPayment) "Caiff eich taliad ymlaen llaw ei gymryd cyn pen 6 diwrnod gwaith. " else ""}Caiff eich taliad nesaf ei gymryd ar $paymentDate neu’r diwrnod gwaith nesaf."
    )
  ...
  object PrintSummary {
    ...
    def `HMRC has agreed to this plan`(amount: String): Message = Message(
      english =
        s"HMRC has agreed to this plan on the understanding that you’ve told us about all your HMRC debts. This plan is only for $amount.",
      welsh =
        s"Mae CThEF wedi cytuno i’r cynllun hwn ar y ddealltwriaeth eich bod wedi rhoi gwybod i ni am eich dyledion gyda CThEF i gyd. Mae’r cynllun ar gyfer $amount yn unig."
    )
    val `You need to make any other payments on time`: Message  = Message(
      ...
      welsh =
        "Mae angen i chi gwneud unrhyw daliadau eraill mewn pryd. Mae angen i chi hefyd anfon bob Ffurflen Dreth atom mewn pryd yn y dyfodol."
    )
  ...
  object NotPaidOnTime {
    ...
    val `If you do not send future tax returns`: Message = Message(
      english =
        "If you do not send future tax returns or pay any tax due on time, we may cancel this plan and ask you to pay in full.",
      welsh =
        "Os na fyddwch yr anfon Ffurflenni Treth neu’n talu unrhyw dreth mewn pryd, mae’n bosibl y byddwn yn canslo’r cynllun hwn a gofyn i chi dalu’n llawn."
    )

    val `We’ve calculated interest at the current rate`: Message = Message(
      english =
        "We’ve calculated interest at the current rate using the amounts and dates agreed. If you pay on any other date, or the interest rate changes, the amount of interest we charge will change.",
      welsh =
        "Rydym wedi cyfrifo llog ar y gyfradd bresennol gan ddefnyddio’r symiau a dyddiadau y cytunwyd arnynt. Os ydych yn talu ar unrhyw ddyddiad arall, neu os yw’r gyfradd llog yn newid, bydd swm y llog byddwn yn ei godi yn newid."
    )

    val `If you’re due a refund from us`: Message = Message(
      english =
        "If you’re due a refund from us while you’re in this plan, we’ll take it off the amount you owe. This will reduce the amount of interest you’ll have to pay.",
      welsh =
        "Os yw ad-daliad yn ddyledus i chi oddi wrthym tra byddwch yn y cynllun hwn, byddwn yn ei dynnu oddi ar y swm sy’n ddyledus gennych. Bydd hyn yn gostwng swm y llog y bydd yn rhaid i chi ei dalu."
    )

    val `If you’ve set up a Direct Debit`: Message = Message(
      english =
        "If you’ve set up a Direct Debit, we’ll continue to collect the original amount unless you give us permission to change it. You can do this by calling us on <strong>0300 123 1813</strong>.",
      welsh =
        "Os ydych wedi trefnu Debyd Uniongyrchol, byddwn yn parhau i gasglu’r swm gwreiddiol oni bai eich bod yn rhoi caniatâd i ni ei newid. Gallwch wneud hyn drwy ffonio ni ar <strong>0300 200 1900</strong>."
    )

    val `If we do not hear from you`: Message = Message(
      english =
        "If we do not hear from you, we’ll put any overpayment at the end of the plan towards any future tax you owe, or send you a refund.",
      welsh =
        "Os na fyddwn yn clywed gennych, byddwn yn rhoi unrhyw ordaliad ar ddiwedd y cynllun tuag at unrhyw dreth bydd arnoch yn y dyfodol neu byddwn yn anfon ad-daliad atoch."
    )

    val `When you make any future Self Assessment payments`: Message = Message(
      english =
        "When you make any future Self Assessment payments, we’ll put these towards your payment plan instead of your latest Self Assessment bill. If you want us to put your payment towards your Self Assessment bill first, call us 5 days after you’ve paid.",
      welsh =
        "Pan fyddwch yn gwneud unrhyw daliadau Hunanasesiad yn y dyfodol, byddwn yn rhoi’r rhain tuag at eich cynllun talu yn hytrach na’ch bil Hunanasesiad diweddaraf. Os ydych am i ni roi eich taliad tuag at eich bil Hunanasesiad yn gyntaf, ffoniwch ni 5 diwrnod ar ôl i chi dalu."
    )
  ...
  object DifficultyPaying {
    ...
    val `We’re here to help`: Message = Message(
      english =
        "We’re here to help. Call us on <strong>0300 123 1813</strong>. Our opening times are Monday to Friday, 8am to 6pm. We are closed on weekends and bank holidays.",
      welsh =
        "Rydym yma i helpu. Ffoniwch ni ar <strong>0300 200 1900</strong>. Ein horiau agor yw Dydd Llun i Ddydd Gwener, 8am i 5:30pm. Rydym ar gau ar benwythnosau a gwyliau banc."
    )
  ...
  object Shuttered {
    ...
    def `You can contact the Payment Support Service...`(link: String): Message = Message(
      english =
        s"""You can <a class="govuk-link" href="$link">contact the Payment Support Service</a> to set up a payment plan by phone.""",
      welsh =
        s"""Gallwch <a class="govuk-link" href="$link">gysylltu â Gwasanaeth Cwsmeriaid Cymraeg CThEF</a> er mwyn sefydlu cynllun talu dros y ffôn."""
    )
