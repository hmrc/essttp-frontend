...
@Singleton
class YourAlreadyHaveDirectDebitController @Inject() (
    as:    Actions,
  ...
  private def displayPage(journey: Journey.AfterEligibilityChecked)(implicit request: Request[_]): Result =
    Ok(
      views.youAlreadyHaveDirectDebit(
        YouAlreadyHaveDirectDebitController.overDuePayments(journey.eligibilityCheckResult),
        journey.taxRegime
      )
    )
...
object YouAlreadyHaveDirectDebitController {
  ...
  def overDuePayments(eligibilityResult: EligibilityCheckResult): OverDuePayments = {
