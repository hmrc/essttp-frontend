...
object CheckPaymentScheduleRows {
  ...
  def canPayWithinSixMonthsRow(
    ...
    canPayWithinSixMonths.map { canPay =>
      SummaryListRow(
        key = Key(
        ...
        value = Value(
          content = Text((if (canPay) { Messages.`Yes` }
                          else Messages.`No`).show)
        ),
        actions = Some(
          Actions(
            items = Seq(
    ...
    (paymentPlan.collections.regularCollections.sortBy(_.dueDate.value): @unchecked) match {
      ...
      case firstCollection +: _ :+ lastCollection =>
        ...
        List(
          ...
          row(Messages.PaymentSchedule.`Start month`, monthAndYear(firstCollection.dueDate)),
          row(
            Messages.PaymentSchedule.`First ... montly payments`(numberOfCollections - 1),
            firstCollection.amountDue.value.gdsFormatInPounds
          ),
