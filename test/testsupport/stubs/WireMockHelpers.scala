...
object WireMockHelpers {
  ...
  /** Useful wiremock helper to verify that the request body serialises to what we expect. Hint: If it's not working and
    * you can't work out why, check what A you are passing in... :)
    * @param url:
    *   String
    * @param format:
    *   play.api.libs.json.Format as an implicit
    * @tparam A:
    *   Model we want to get format for
    */
  def verifyWithBodyParse[A](url: String)(using Format[A]): Unit = verify(
  ...
  /** Same as above, but it also compares a Json serialised expected A with the wiremock request
    */
  def verifyWithBodyParse[A](url: String, expected: A)(using Format[A]): Unit = verify(
    postRequestedFor(urlPathEqualTo(url))
      .andMatching((value: Request) => customValueMatcher(url, value))
  ...
  /** Same as above, but overloaded with headers to check
    */
  def verifyWithBodyParse[A](url: String, headers: (String, String))(using Format[A]): Unit = verify(
    postRequestedFor(urlPathEqualTo(url))
  ...
  /** Helper method to attempt to parse request body into type, using asOpt. If option is empty, it can't and wiremock
    ...
    */
  private def customValueMatcher[A](url: String, request: Request)(using Format[A]): MatchResult =
  ...
  def stubForPostWithResponseBody(url: String, jsonBody: String, responseStatus: Int = Status.OK): StubMapping =
    stubFor(
      post(urlPathEqualTo(url)).willReturn(
        aResponse()
          .withStatus(responseStatus)
