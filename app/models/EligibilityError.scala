...
object EligibilityErrors extends Enum[EligibilityError] {
  ...
  def toEligibilityError(eligibilityRules: EligibilityRules): Option[EligibilityError] = {
    ...
    normalisedEligibilityRules match {
      ...
      case EligibilityRules(true, _, _, _, _, _, _, _, _, _, _, _, _)             => Some(HasRlsOnAddress)
      case EligibilityRules(_, true, _, _, _, _, _, _, _, _, _, _, _)             => Some(MarkedAsInsolvent)
      case EligibilityRules(_, _, true, _, _, _, _, _, _, _, _, _, _)             => Some(IsLessThanMinDebtAllowance)
      case EligibilityRules(_, _, _, true, _, _, _, _, _, _, _, _, _)             => Some(IsMoreThanMaxDebtAllowance)
      case EligibilityRules(_, _, _, _, true, _, _, _, _, _, _, _, _)             => Some(DisallowedChargeLockTypes)
      case EligibilityRules(_, _, _, _, _, true, _, _, _, _, _, _, _)             => Some(ExistingTtp)
      case EligibilityRules(_, _, _, _, _, _, true, _, _, _, _, _, _)             => Some(ChargesOverMaxDebtAge)
      case EligibilityRules(_, _, _, _, _, _, _, true, _, _, _, _, _)             => Some(IneligibleChargeTypes)
      case EligibilityRules(_, _, _, _, _, _, _, _, true, _, _, _, _)             => Some(MissingFiledReturns)
      case EligibilityRules(_, _, _, _, _, _, _, _, _, Some(true), _, _, _)       => Some(HasInvalidInterestSignals)
      case EligibilityRules(_, _, _, _, _, _, _, _, _, _, Some(true), _, _)       => Some(DmSpecialOfficeProcessingRequired)
      case EligibilityRules(_, _, _, _, _, _, _, _, _, _, _, true, _)             => Some(NoDueDatesReached)
      case EligibilityRules(_, _, _, _, _, _, _, _, _, _, _, _, Some(true))       => Some(CannotFindLockReason)
      case EligibilityRules(_, _, _, _, _, _, _, _, _, _, _, _, _)                => None //all false
    }
  }
