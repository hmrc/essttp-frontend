...
class LanguageSwitchController @Inject() (
  ...
  appConfig: AppConfig)(implicit ec: ExecutionContext) extends AbstractController(cc) with I18nSupport {
  ...
  def switchToLanguage(language: Language): Action[AnyContent] = cc.actionBuilder { implicit request =>
    val result: Result = request.headers.get(HeaderNames.REFERER) match {
      case Some(referrer) =>
        ...
        else Redirect(controllers.routes.IndexController.onPageLoad)
